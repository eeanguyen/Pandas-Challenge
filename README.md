...
Our analysis reveals a surprising correlation between charter schools and overall passing rate, but this doesn't necessarily suggest that academic sucess is greater in comparison to district schools. It's important to note that charter schools are smaller in size compared to district schools. For example, when we refer to the data frame that categorizes the schools with smaller student count we can see that schools like Pena High School and Holden High School have the some of the higher overall passing rates (with > 89% overall passing) but are on the less populated end of the total student count. This can give insight that since there are less students, that instructors and faculty have more bandwidth to ensure each student is getting the aid they need for academic success. Moving forward, some of the smaller populated charter schools may have skewed the data since they are outliers in comparison to the rest of the data.

In the bottom performing schools (refer to Bottom Performing Schools DataFrame) there seems to be a great discrepancy between % passing math and % passing reading. With this information, we can consider making a decision on using some of the schools funding to aid the mathematics department in order to get students the supplementary instruction they need in order to increase their performance in math classes. This can be applied to all schools in the district since math can be seen to be more challenging for most students based on the comparable passing rates between Reading and Math. Another great take away from this data is that we notice that schools that are consdiered large (2,000-5,000 students) have a lower overall passing rate. Another observation that can give way that the faculty could have more attention to each and every student rather than large classes where half of the students are left behind on the study material.

Overall, it's applaudable all the schools in the district have over 50% overall passing rates! In the near future, it is possible that some schools may want to redistribute their per capita budget to the mathematics department in order to increase passing rates in the subject. Another strong consideration is to possibly redesign the school assignment layouts within communities. The schools that are higher in population could be doing a disservice to both the students and faculty by overpopulating a single school. There may even be discussions on possibly lowering Charter school tuition for students so that more children have the oppourtunity to get a more tailored education geared towards overall student success.
